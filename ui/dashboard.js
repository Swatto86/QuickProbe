        ...
        const HEARTBEAT_STALE_MS = 360000;
        const QUICK_PROBE_MIN_INTERVAL_MS = 60000;
        ...
        function showConfirmDialog(message, options = {}) {
            return new Promise((resolve) => {
                ...
                // If modal doesn't exist, create a themed one dynamically (should never happen)
                if (!modal) {
                    ...
                    modal.innerHTML = `
                        <div class="modal-box">
                            <h3 id="confirm-modal-title-dynamic" class="font-bold text-lg">Confirm</h3>
                            <p id="confirm-modal-message-dynamic" class="py-4 text-base-content/80 whitespace-pre-wrap"></p>
                            <div class="modal-action">
                                <button type="button" id="confirm-modal-cancel-dynamic" class="btn">Cancel</button>
                                <button type="button" id="confirm-modal-ok-dynamic" class="btn btn-primary">OK</button>
                            </div>
                        </div>
                        <div class="modal-backdrop bg-black/50" id="confirm-modal-backdrop-dynamic"></div>
                    `;
                    document.body.appendChild(modal);
                    titleEl = document.getElementById('confirm-modal-title-dynamic');
                    messageEl = document.getElementById('confirm-modal-message-dynamic');
                    okBtn = document.getElementById('confirm-modal-ok-dynamic');
                    cancelBtn = document.getElementById('confirm-modal-cancel-dynamic');
                    backdrop = document.getElementById('confirm-modal-backdrop-dynamic');
                }
        ...
        /// Show or hide the "Local Mode" badge in the dashboard header.
        async function hydrateLoginModeBadge() {
            const badge = document.getElementById('local-mode-badge');
            if (!badge) return;
            try {
                const mode = await window.__TAURI__.core.invoke('get_login_mode');
                if (mode === 'local') {
                ...
                console.warn('Failed to load login mode', err);
                badge.classList.add('hidden');
            }
        }
        ...
        function markHostsChanged() {
            ...
            settingsBundle.qp_hosts_changed = stamp;
            try {
                localStorage.setItem(HOSTS_CHANGED_KEY, JSON.stringify(stamp));
            } catch (_) { /* ignore */ }
            queueSettingsSave(settingsPayload());
        }

        function consumeHostsChangedFlag() {
            let flag = settingsBundle?.qp_hosts_changed;
            if (!flag) {
                try {
                    const raw = localStorage.getItem(HOSTS_CHANGED_KEY);
            ...
            try {
                localStorage.removeItem(HOSTS_CHANGED_KEY);
            } catch (_) { /* ignore */ }
            if (flag) {
                queueSettingsSave(settingsPayload());
                return true;
        ...
        document.addEventListener('DOMContentLoaded', async () => {
            ...
            try {
                ...
                window.addEventListener('storage', async (e) => {
                    if (e.key === SETTINGS_KEY || e.key === HOSTS_CHANGED_KEY) {
                        try {
                ...
                const signatureMismatch = Boolean(
                    previousSignature && latestSignature && latestSignature !== previousSignature
                );

                // Rebuild from latest host list when explicitly flagged or when signature drift is detected.
                // This prevents stale card metadata when the host-change flag is missed/delayed.
                if (hostsChanged || signatureMismatch) {
                    hostsSignature = latestSignature;
        ...
        function renderEditServiceBrowserTable() {
            ...
            document.querySelectorAll('#edit-service-browser-modal th[data-sort]').forEach(th => {
                ...
                if (th.dataset.sort === editServiceBrowserSortKey) {
                    indicator.textContent = editServiceBrowserSortAsc ? ' ▲' : ' ▼';
                } else {
        ...
        function initDetailsDialog() {
            ...
            openDetailsDialog = (server) => {
                ...
                const errorLines = errors.length ? errors.map(e => {
                    ...
                    return `${e.log || 'Log'} ${e.id || ''} ${e.provider || ''} @ ${when} — ${e.message || ''}`;
                }) : ['No recent errors'];
                ...
                if (Array.isArray(data.high_cpu_processes) && data.high_cpu_processes.length) {
                    ...
                    data.high_cpu_processes.forEach(p => {
                        ...
                        uptimeCpuLines.push(` • ${p.name || 'Process'} (${pct})`);
                    });
                }
                ...
                if (titleEl) {
                    titleEl.textContent = `Details – ${server.name}`;
                }
        ...
        async function hydrateServerWithHealth(serverInfo, invoke) {
            ...
            const fetchAdapters = async () => {
                try {
                    ...
                    // Swallow; we’ll fall back to existing adapters if present.
                }
                return null;
            };
            try {
        ...
        async function loadAllServers(options = {}) {
            ...
            const runId = ++loadAllServersRunId;

            logDebug(`dashboard:loadAllServers START run=${runId} silent=${silent} quickOnly=${quickOnly}`);
            ...
            try {
                ...
                if (runId !== loadAllServersRunId) {
                    logDebug(`dashboard:loadAllServers ABORT run=${runId} superseded after host fetch`);
                    return;
                }
                ...
                if (runId !== loadAllServersRunId) {
                    logDebug(`dashboard:loadAllServers ABORT run=${runId} superseded before base apply`);
                    return;
                }
                ...
                const probedServers = await mapWithConcurrency(
                    hosts,
                    MAX_CONCURRENT_PROBES,
                    (serverInfo) => hydrateServerWithHealth(serverInfo, invoke)
                );
                if (runId !== loadAllServersRunId) {
                    logDebug(`dashboard:loadAllServers ABORT run=${runId} superseded after probe`);
                    return;
                }
                serversData = applyOrder(probedServers, order);
                cacheDashboardData();
                displayAllServers();
                startHeartbeatLoop(heartbeatHosts);
        ...
        function renderGroupView(servers, container) {
            ...
            entries.forEach(([groupName, hosts]) => {
                ...
                nameBtn.innerHTML = `<span class="text-2xl">📁</span> ${escapeHtml(canonicalName)}`;
                ...
                sub.textContent = `${hosts.length} host${hosts.length === 1 ? '' : 's'} • click to expand`;
            ...
            if (reorderMode) {
                ...
                banner.innerHTML = `
                    <div class="banner-text">
                        <span>↕️</span>
                        <span>Reorder Mode — Drag cards to rearrange</span>
                    </div>
        ...
        async function refreshVisibleHosts(buttonEl) {
            ...
            if (buttonEl) {
                ...
                buttonEl.textContent = `🔄 Refreshing (0/${total})...`;
            }
            ...
            const bumpProgress = () => {
                completed += 1;
                if (buttonEl) {
                    buttonEl.textContent = `🔄 Refreshing (${completed}/${total})...`;
                }
            };
        ...
        function createServerCard(server, options = {}) {
            ...
            if (normalized && refreshState?.inFlight) {
                card.classList.add('refreshing');
                console.log(`[createServerCard] ✓ Preserving refreshing animation for ${server.name}`, {
                    inFlight: refreshState.inFlight,
                    hasClass: card.classList.contains('refreshing')
                });
            } else if (normalized && refreshState) {
                console.log(`[createServerCard] ✗ NOT preserving for ${server.name}`, {
                    inFlight: refreshState.inFlight,
            ...
            const groupMarkup = `<div class="flex gap-2 flex-wrap mb-1">${osMarkup}${groupValue ? `<span class="badge badge-secondary font-semibold gap-1">📁 ${escapeHtml(groupValue)}</span>` : ''}</div>`;
            ...
            const locationLine = `
                <div class="card-details flex items-center gap-2 flex-wrap my-2">
                    <span class="text-warning font-bold">📍 Location:</span>
            ...
            if (!server.online) {
                ...
                const noteMarkup = server.notes ? `<div class="text-base-content/60 text-sm mb-3 italic">📝 ${highlightMatch(server.notes, searchTerm)}</div>` : '';
                ...
                const pendingBadge = data.pending_reboot?.pending ? `<span class="badge badge-warning gap-1" title="${escapeHtml(pendingTooltip)}">🔄 Reboot Pending</span>` : '';

                const errorsMarkup = Array.isArray(data.recent_errors) && data.recent_errors.length > 0 ? `
                    <div class="card-details mt-2">
                        <div class="metric-label text-error font-semibold">⚠️ Recent Critical/Errors (last 30m)</div>
                ...
                card.innerHTML = `
                    ...
                    <div class="server-card-header flex justify-between items-center gap-2 flex-wrap">
                        ...
                        <div class="flex gap-2 items-center flex-wrap">
                            ${statusBadge}
                            ${pendingBadge}
                            <span class="badge badge-error gap-1">✗ OFFLINE</span>
                        </div>
                    </div>
                    ${groupMarkup}
                    ${noteMarkup}
                    <div class="last-refresh flex gap-2 flex-wrap items-center">
                        <span class="badge ${quickStale ? 'badge-warning' : 'badge-success'} gap-1" title="Full refresh: ${escapeHtml(lastRefreshLabel)}">⏱️ Quick: ${escapeHtml(quickLabelWithStale)}</span>
                    </div>
                    ${reachabilityMarkup}
                    ${locationLine}
                    <!-- Always visible: Basic system info -->
                    <div class="flex gap-4 flex-wrap mb-1">
                        <div><span class="text-xs text-info font-semibold">⏱️ Uptime:</span> <span class="font-bold text-sm text-primary">${highlightMatch(uptimeLabel, searchTerm)}</span></div>
                        <div><span class="text-xs text-info font-semibold">🖥️ CPU Load:</span> <span class="font-bold text-sm ${cpuValue >= 90 ? 'text-error' : cpuValue >= 70 ? 'text-warning' : 'text-success'}">${highlightMatch(cpuLoadLabel, searchTerm)}</span></div>
                    </div>
                    <div class="flex gap-4 flex-wrap mb-1">
                        <div><span class="text-xs text-secondary font-semibold">💿 OS Version:</span> <span class="font-bold text-sm text-accent">${highlightMatch(data.os_info?.os_version || 'Unknown', searchTerm)}</span></div>
                    </div>
                    <div class="flex gap-4 flex-wrap mb-1">
                        <div><span class="text-xs text-info font-semibold">🧠 Total RAM:</span> <span class="font-bold text-sm text-primary">${totalMemGb > 0 ? `${totalMemGb} GB` : 'Unknown'}</span></div>
                        <div><span class="text-xs text-info font-semibold">📊 RAM Usage:</span> <span class="font-bold text-sm ${memPercent >= 80 ? 'text-error' : memPercent >= 65 ? 'text-warning' : 'text-success'}">${usedMemGb > 0 ? `${usedMemGb} GB` : 'Unknown'} (${Math.round(memPercent)}%)</span></div>
                    </div>
                    <!-- Hidden until expanded: Detailed info -->
                    <div class="card-details flex gap-4 flex-wrap mb-1">
                        <div><span class="text-xs text-accent font-semibold">⚙️ Processes:</span> <span class="font-bold text-sm text-primary">${processCount}</span></div>
                        <div><span class="text-xs text-accent font-semibold">🔥 High CPU (&gt; ${highCpuThreshold}%):</span> <span class="font-bold text-sm ${highCpuCount > 0 ? 'text-warning' : 'text-success'}">${highCpuCount > 0 ? highCpuNames : 'None'}</span></div>
                    </div>
                    <div class="card-details flex flex-col gap-1 mb-2">
                        <div><span class="text-xs text-secondary font-semibold">🌐 IP:</span> <span class="font-bold text-sm text-info">${highlightMatch(primaryIp, searchTerm)}</span></div>
                        <div><span class="text-xs text-secondary font-semibold">📡 Subnet:</span> <span class="font-bold text-sm text-info">${highlightMatch(primarySubnet, searchTerm)}</span></div>
                        <div><span class="text-xs text-secondary font-semibold">🚪 Gateway:</span> <span class="font-bold text-sm text-info">${highlightMatch(primaryGateway, searchTerm)}</span></div>
                        <div><span class="text-xs text-secondary font-semibold">🔍 DNS:</span> <span class="font-bold text-sm text-info">${highlightMatch(primaryDns, searchTerm)}</span></div>
                    </div>
                    <div class="server-services card-details">
                        <div class="metric-label text-primary font-semibold">🔧 Services Status:</div>
                        <div class="services-summary">
                            <span class="text-success font-semibold">✓ ${runningServices.length} Running</span>
                            ${stoppedServices.length > 0 ?
                        `<span class="text-error font-semibold">✗ ${stoppedServices.length} Stopped</span>` :
                    ...
                    <div class="card-details mt-3">
                        <div class="metric-label text-primary font-semibold">💾 Disk Space:</div>
                        <div class="text-sm text-base-content/90 leading-relaxed">
                            ...
                            return `<div>${highlightMatch(diskLabel, searchTerm)} <span class="${colorClass}">●</span></div>`;
                        }).join('')}
                        </div>
                    </div>` : ''}
                    ${errorsMarkup}
                    <div class="qp-card-actions">
                        <div class="qp-card-actions-row">
                            ...
                            <div class="dropdown dropdown-end">
                                <button tabindex="0" class="manage-btn">⚙️ Actions</button>
                                <ul tabindex="0" class="dropdown-content menu bg-base-200 rounded-box z-50 min-w-[28rem] p-2 shadow-lg border border-base-300">
                                    <li><a class="rdp-creds-btn whitespace-nowrap">🔑 Set Host Credentials</a></li>
                                    <li><hr class="my-2 border-t-2 border-base-300 opacity-80"></li>
                                    <li class="menu-title text-xs opacity-80 font-bold">Remote Access</li>
                                    <li><a class="remote-shell-btn whitespace-nowrap">${isLinux ? '🐧 Remote SSH' : '💻 Remote PowerShell'}</a></li>
                                    ${!isLinux ? '<li><a class="explore-share-btn whitespace-nowrap">📁 Explore C$</a></li>' : ''}
                                    <li><hr class="my-2 border-t-2 border-base-300 opacity-80"></li>
                                    <li class="menu-title text-xs opacity-80 font-bold">Management</li>
                                    <li><a class="manage-services-btn whitespace-nowrap">${isLinux ? '🐧 Manage Services (systemd)' : '🔧 Manage Services'}</a></li>
                                    <li><a class="manage-processes-btn whitespace-nowrap">${isLinux ? '🐧 Manage Processes (top)' : '📊 Manage Processes'}</a></li>
                                    ${!isLinux ? `
                                    <li><hr class="my-2 border-t-2 border-base-300 opacity-80"></li>
                                    <li class="menu-title text-xs opacity-80 font-bold">System Tools</li>
                                    <li><a class="remote-registry-btn whitespace-nowrap">🗝️ Remote Registry</a></li>
                                    <li><a class="stop-remote-registry-btn whitespace-nowrap">🗝️ Stop Remote Registry</a></li>
                                    ` : ''}
                                    <li><hr class="my-2 border-t-2 border-base-300 opacity-80"></li>
                                    <li class="menu-title text-xs opacity-80 font-bold">Power Management</li>
                                    <li><a class="restart-server-btn whitespace-nowrap">🔄 Restart Server</a></li>
                                    <li><a class="shutdown-server-btn whitespace-nowrap">⏹️ Shutdown Server</a></li>
                                </ul>
                            </div>
                        </div>
                        <div class="qp-card-actions-row">
                            <button class="edit-host-btn">✏️ Edit</button>
                        </div>
                    </div>
                    </div>
                `;
                ...
                // Remote Registry button handler
                const remoteRegistryBtn = card.querySelector('.remote-registry-btn');
                if (remoteRegistryBtn) {
                    remoteRegistryBtn.addEventListener('click', (e) => {
                        e.stopPropagation();
                        e.preventDefault();
                        startRemoteRegistry(server.name);
                    });
                }

                // Stop Remote Registry button handler
                const stopRemoteRegistryBtn = card.querySelector('.stop-remote-registry-btn');
                if (stopRemoteRegistryBtn) {
                    stopRemoteRegistryBtn.addEventListener('click', (e) => {
                        e.stopPropagation();
                        e.preventDefault();
                        stopRemoteRegistry(server.name);
                    });
                }

                // === Remote Actions (Cross-platform) ===

                // Restart Server button handler
                const restartServerBtn = card.querySelector('.restart-server-btn');
                if (restartServerBtn) {
                ...
                // Shutdown Server button handler
                const shutdownServerBtn = card.querySelector('.shutdown-server-btn');
                if (shutdownServerBtn) {
                    shutdownServerBtn.addEventListener('click', (e) => {
                        e.stopPropagation();
                        e.preventDefault();
                        shutdownServer(server.name);
                    });
                }
            ...
            const winrmBadge = (!isLinux && isWinrmIssue) ? '<span class="badge badge-warning gap-1">⚠️ WINRM ISSUE</span>' : '';
            const onlineBadge = '<span class="badge badge-success gap-1">✓ ONLINE</span>';
            ...
            card.innerHTML = `
                ...
                ${groupMarkup}
                ${server.notes ? `<div class="text-base-content/60 text-sm mb-3 italic">📝 ${highlightMatch(server.notes, searchTerm)}</div>` : ''}
                <div class="last-refresh flex gap-2 flex-wrap items-center">
                    <span class="badge ${quickStale ? 'badge-warning' : 'badge-success'} gap-1" title="Full refresh: ${escapeHtml(lastRefreshLabel)}">⏱️ Quick: ${escapeHtml(quickLabel)}</span>
                </div>
                ${reachabilityMarkup}
                ${locationLine}
                <!-- Always visible: Basic system info -->
        ...
        async function editServerNotes(serverName, currentNotes) {
            ...
            try {
                ...
                loadAllServers({ silent: true, quickOnly: true });
            } catch (error) {
        ...
        async function deleteHostFromDashboard(serverName) {
            ...
            try {
                ...
                heartbeatHosts = (heartbeatHosts || []).filter((host) => {
                    const hostName = normalizeHostName(host?.name || '');
                    return hostName && hostName !== normalizeHostName(serverName);
                });
                stopHeartbeatLoop();
                startHeartbeatLoop(heartbeatHosts);
                cacheDashboardData();
        ...
        function renderReachabilityRow(server) {
            ...
            const pingPill = `<span class="badge badge-sm ${pingOk ? 'badge-success' : 'badge-error'}" title="ICMP Ping ${pingOk ? 'succeeded' : 'failed'}">Ping: ${pingOk ? '✓' : '✗'}</span>`;
            ...
            const tcpPills = tcpPorts.map(p => {
                ...
                return `<span class="badge badge-sm ${ok ? 'badge-success' : 'badge-error'}" title="${escapeHtml(tooltip)}">${label}: ${ok ? '✓' : '✗'}</span>`;
            }).filter(Boolean).join('');
        ...
        /**
         * Start the RemoteRegistry service and launch regedit for remote connection
         * @param {string} serverName - The target server name
         */
        async function startRemoteRegistry(serverName) {
            if (!serverName) {
                showError('Server name is required');
                return;
            }
            ...
            try {
                ...
                // First, start the RemoteRegistry service
                showInfo(`Starting Remote Registry service on ${serverName}...`);
                ...
                // Wait for service to fully initialize (3 seconds)
                showInfo(`Waiting for RemoteRegistry service to initialize...`);
                await new Promise(resolve => setTimeout(resolve, 3000));

                // Then launch regedit
                await invoke('launch_remote_registry', { server: serverName });

                showInfo(`Connected to Remote Registry on ${serverName}`);
            } catch (err) {
                const message = err?.toString?.() ? err.toString() : `${err}`;
                showError(`Failed to start Remote Registry: ${message}`);
                logDebug(`startRemoteRegistry failed for ${serverName}: ${message}`);
            }
        }

        /**
         * Stop the RemoteRegistry service on a remote server
         * @param {string} serverName - The target server name
         */
        async function stopRemoteRegistry(serverName) {
            if (!serverName) {
                showError('Server name is required');
                return;
            }
            ...
            const confirmed = await showConfirmDialog(
                `Stop the Remote Registry service on ${serverName}?`,
                {
            ...
            try {
                ...
                showInfo(`Remote Registry service stopped on ${serverName}`);
            } catch (err) {
                const message = err?.toString?.() ? err.toString() : `${err}`;
                showError(`Failed to stop Remote Registry: ${message}`);
                logDebug(`stopRemoteRegistry failed for ${serverName}: ${message}`);
            }
        }
        ...
        /**
         * Show a countdown timer with cancel button before executing a critical action
         * @param {string} message - Message to display during countdown
         * @param {number} seconds - Countdown duration in seconds (default: 10)
         * @returns {Promise<boolean>} - True if countdown completed, false if cancelled
         */
        async function showCountdownDialog(message, seconds = 10) {
            return new Promise((resolve) => {
                ...
                container.className = 'fixed top-4 right-4 z-[9999] bg-warning text-warning-content p-6 rounded-lg shadow-2xl border-2 border-warning-content/20 min-w-[400px]';
                container.style.animation = 'slideInRight 0.3s ease-out';

                const messageDiv = document.createElement('div');
                messageDiv.className = 'font-bold text-lg mb-3';
                messageDiv.textContent = message;

                const countdownDiv = document.createElement('div');
                countdownDiv.className = 'text-6xl font-mono text-center my-4';
                countdownDiv.textContent = timeLeft;

                const progressBar = document.createElement('div');
                progressBar.className = 'w-full h-2 bg-warning-content/20 rounded-full overflow-hidden mb-4';
                const progressFill = document.createElement('div');
                progressFill.className = 'h-full bg-warning-content transition-all duration-1000 ease-linear';
                progressFill.style.width = '100%';
                progressBar.appendChild(progressFill);

                const cancelBtn = document.createElement('button');
                cancelBtn.className = 'btn btn-error btn-block btn-lg';
                ...
                intervalId = setInterval(() => {
                    ...
                    // Change color in final 3 seconds
                    if (timeLeft <= 3) {
                        container.className = container.className.replace('bg-warning', 'bg-error');
                        container.className = container.className.replace('text-warning-content', 'text-error-content');
                        countdownDiv.className = countdownDiv.className + ' animate-pulse';
                    }
        ...
        async function restartServer(serverName) {
            ...
            const confirmed = await showConfirmDialog(
                `Are you sure you want to restart ${serverName}? This will interrupt all active sessions and services.`,
                {
                    title: 'Confirm Remote Restart',
                    okText: 'Proceed',
                    okClass: 'btn-warning'
                }
            );
            ...
            // Show countdown timer with cancel option
            const countdownCompleted = await showCountdownDialog(
                `⏱️ Restarting ${serverName} in...`,
                10
            );

            if (!countdownCompleted) {
                showInfo('Restart cancelled');
                return;
            }
            ...
            try {
                const { invoke } = window.__TAURI__.core;
                await invoke('remote_restart', { serverName });
                showInfo(`Restart initiated for ${serverName}`);

                // Trigger a refresh after a short delay to update status
                setTimeout(() => triggerQuickProbe(serverName, 'restart-check'), 3000);
            } catch (err) {
                const message = err?.toString?.() ? err.toString() : `${err}`;
                showError(`Failed to restart ${serverName}: ${message}`);
                logDebug(`remote_restart failed for ${serverName}: ${message}`);
            }
        }
        ...
        async function shutdownServer(serverName) {
            ...
            // First confirmation dialog with severe warning
            const confirmed = await showConfirmDialog(
                `⚠️ WARNING: Are you sure you want to SHUT DOWN ${serverName}? The server will need to be physically or remotely powered on again. This will interrupt all active sessions and services.`,
                {
                    title: 'Confirm Remote Shutdown',
                    okText: 'Proceed',
                    okClass: 'btn-error'
                }
            );
            ...
            // Show countdown timer with cancel option
            const countdownCompleted = await showCountdownDialog(
                `⏱️ Shutting down ${serverName} in...`,
                10
            );

            if (!countdownCompleted) {
                showInfo('Shutdown cancelled');
                return;
            }
            ...
            try {
                const { invoke } = window.__TAURI__.core;
                await invoke('remote_shutdown', { serverName });
                showInfo(`Shutdown initiated for ${serverName}`);

                // Trigger a refresh after a short delay to update status
                setTimeout(() => triggerQuickProbe(serverName, 'shutdown-check'), 3000);
            } catch (err) {
                const message = err?.toString?.() ? err.toString() : `${err}`;
                showError(`Failed to shutdown ${serverName}: ${message}`);
                logDebug(`remote_shutdown failed for ${serverName}: ${message}`);
            }
        }
                ...
                return `
                    <tr class="hover:bg-base-200">
                        <td class="font-mono text-sm">
                            ${isMonitored ? '<span class="badge badge-primary badge-xs mr-1" title="Monitored service">👁</span>' : ''}
                            ${escapeHtml(svc.name)}
                        </td>
                        <td>${escapeHtml(svc.display_name)}</td>
                        ...
                        <td class="text-center">
                            <div class="flex gap-1 justify-center">
                                ...
                                <button class="btn btn-xs btn-error svc-stop-btn"
                                    data-service="${escapeHtml(svc.name)}"
                                    ${isStopped ? 'disabled' : ''}
                                    title="Stop service">⏹</button>
                                <button class="btn btn-xs btn-warning svc-restart-btn"
                                    data-service="${escapeHtml(svc.name)}"
                                    title="Restart service">🔄</button>
                            </div>
                        </td>
                    </tr>
                `;
            ...
            if (thead) {
                thead.querySelectorAll('th[data-sort]').forEach(th => {
                    th.addEventListener('click', () => {
                        ...
                        const indicator = th.querySelector('.sort-indicator');
                        if (indicator) {
                            indicator.textContent = manageServicesSortAsc ? ' ▲' : ' ▼';
                        }
                        renderManageServicesTable();
                    });
                });
                    ...
                    <tr class="hover:bg-base-200">
                        ...
                        <td class="text-center">
                            <button class="btn btn-xs btn-error proc-kill-btn"
                                ...
                                title="Kill process">☠️ Kill</button>
                        </td>
                    </tr>
                `;
            }).join('');
            ...
            if (thead) {
                thead.querySelectorAll('th[data-sort]').forEach(th => {
                    th.addEventListener('click', () => {
                        ...
                        const indicator = th.querySelector('.sort-indicator');
                        if (indicator) {
                            indicator.textContent = manageProcessesSortAsc ? ' ▲' : ' ▼';
                        }
                        renderManageProcessesTable();
                    });
                });
        ...
        async function runUploadedScript() {
            ...
            scriptHeader.textContent = `📄 Running script: ${remotePowerShellScriptName} on ${remotePowerShellServerName}`;
            ...
            try {
                ...
                footer.textContent = `${result.success ? '✅' : '❌'} Script completed in ${elapsed}s`;
